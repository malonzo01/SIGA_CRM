...
@section('content')
    ...
    <section class="section dashboard">
      <div class="row">
            ...
            @include('admin.categories.modals.confirm')
                    ...
                    <div class="card-header bg-primary text-white">
                        <div class="row">
                            ...
                            <div class="col-md-1 col-sm-12">
                                <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#ModalCategoryCrear" style="padding:5px;" title="Nuevo">
...
<script>
    ...
    $('#form_categories_post').on('submit', function(event){
           ...
           $.ajax({
            ...
            if(data.error)
            {
             ...
             if(data.success)
             {
                ...
                $('#form_categories_post')[0].reset();
                $('#table').DataTable().ajax.reload();
                $('#ModalCategoryCrear').modal('hide');
             }
        ...
        $(document).on('click', '.edit', function(){
              ...
              $.ajax({
               ...
               success:function(html){
                $('#orn_cat_edit').val(html.data.orn_cat);
                $('#name_edit').val(html.data.name);
                $('#description_edit').val(html.data.description);
                $('#status_edit').val(html.data.status);
                $('#id_category').val(html.data.id);
                $('#ModalCategoryEditar').modal('show');
               }
              })
        });

        $('#form_categories_update').on('submit', function(event){
            event.preventDefault();
             ...
             $.ajax({
                ...
                if(data.error)
                {
                 ...
                 {
                  html = '<div class="alert alert-dismissible alert-success"><button type="button" class="btn-close" data-bs-dismiss="alert"></button>' + data.success + '</div>';
                  $('#ModalCategoryEditar').modal('hide');
